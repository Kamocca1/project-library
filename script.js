...
class Book {
    #read = false;
    constructor(id, title, author, pages, imageUrl) {
        this.id = id;
        this.title = title;
        this.author = author;
        this.pages = pages;
    ...
    set read(value) {
        if (typeof value !== "boolean") {
            throw new TypeError("Read must be a boolean value.");
        }
        this.#read = value;
        this.#read = !this.#read;
    }
...
function addBookToLibrary(title, author, pages, imageUrl) {
    const id = crypto.randomUUID();
    const intPages = parseInt(pages);
    const book = new Book(id, title, author, intPages, imageUrl);
    myLibrary.push(book);
    displayBooks();
}
...
addBookToLibrary(
    ...
    "George Orwell",
    328,
    "https://upload.wikimedia.org/wikipedia/commons/5/51/1984_first_edition_cover.jpg?20230923071852"
);
addBookToLibrary(
    "To Kill a Mockingbird",
    "Harper Lee",
    281,
    "https://upload.wikimedia.org/wikipedia/commons/thumb/4/4f/To_Kill_a_Mockingbird_%28first_edition_cover%29.jpg/640px-To_Kill_a_Mockingbird_%28first_edition_cover%29.jpg"
);
